...
dat2$top1 <- sin(dat2$slopeR)  # steepness of slope
dat2$top2 <- sin(dat2$slopeR) * sin(dat2$aspectR)  # East-west directionality (I think?)
dat2$top3 <- sin(dat2$slopeR) * cos(dat2$aspectR)  # North-south directionality
# create variable indicating NE-SW directionality
dat2$NE <- sin(dat2$slopeR) * cos(dat2$aspectR - as_radians(45))

# create proportion conspecific variable
dat2$propCon <- with(dat2, plot_cons_tree_BA / plot_tree_BA)
...
dat4 <- dat4 %>% filter(! species %in% to_remove)
